...
function isSelected(urlName) {
  const { params } = this.props.match
  return urlName === params.urlName
}
...
export default class Search extends PureComponent {
  ...
  render() {
    ...
    const { isSelected, placeholder } = this.props
    ...
    const elems = values.map(({ _id, base, vars }) => {
      ...
      return (
        <Link
          ...
          data-selected={isSelected(urlName)}
        >
    ...
    return (
      <Block>
        ...
        {hover && <List onMouseDown={this.handle}>{elems}</List>}
      </Block>
    )
  }
  ...
  handle = e => {
    ...
    const target = this.state.values.find(
      value => value.base.urlName === urlName,
    )

    if (target) this.props.handle(target)
  }
}
